...
#ifdef OVERLOAD_R4
  use constantsR4_mod,      only: kappa, rdgas, rvgas, grav, cp_air, cp_vapor, pi=>pi_8, TFREEZE, wtmair, wtmh2
#else
  use constants_mod,      only: kappa, rdgas, rvgas, grav, cp_air, cp_vapor, pi=>pi_8,TFREEZE, wtmair, wtmh2
#endif
...
#ifdef MULTI_GASES
                adj_vmr(i,j,k) = Mw_air(sum(q(i,j,k,1:nn)))/Mw_air(sum(q(i,j,k,1:nn))-dt*sum(q_dt(i,j,k,1:nn)))
